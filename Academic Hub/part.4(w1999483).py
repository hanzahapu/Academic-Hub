...
def get_input():
    ...
    while True:
        try:
            ...
            if f_credit not in [0,20,40,60,80,100,63,120]:
                print("Out of range")
            else:
                break
        except ValueError:
...
def repeat_run():
    ...
    #Histogram
            ...
            text = open("details.txt","w")
            
            if add1>=1:
                for a in range(0,len(list1),1):
                    text.write("\nProgress -                    " + str(list1[a:a+1])[2:-2])
            if add2>=1:
                for a in range(0,len(list2),1):
                    text.write("\nProgress (Module Trailer) -   " + str(list2[a:a+1])[2:-2])
            if add3>=1:
                for a in range(0,len(list3),1):
                    text.write("\nModule Retriever -            " + str(list3[a:a+1])[2:-2])
            if add4>=1:
                for a in range(0,len(list4),1):
                    text.write("\nExclude -                     " + str(list4[a:a+1])[2:-2])
...
def id_no():
    print("\n")
    global idnum 
    idnum= input("Enter your University ID: ")
